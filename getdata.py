...
apiKey = ['3ce9e0ab3ce7491b896c2fff0aae03ea','54b57422e2d14f169dd132ca36c669f2','6f0e158decfb4262a93248b4c5337616','fc0b654bc64a47c0a4d0fa880acbaf6b','25b072e7437e4aeea6e1eba44d3fce7f','0e2b90f581084acd94162753f5c8ba25','6c6ba1e5abec480abcfd97889e46ef41','e3e321b663fa43f581cc4cfd922ff9a1','74571be91b5544e38860a4c8baf7bfda','efbd8a0c9a2a41fa8d1cc365db79a39a']
...
def relevance(df,ticker):
    for index, row in df.iterrows():
        if((((str(['ticker'])).lower()) in ((str(row['title'])).lower())) or (((str(['companyName'])).lower()) in ((str(row['title'])).lower()))):
            ...
            df.loc[index, 'r' ] = -1 # consider dropping 
    return df

def request(ticker,date):

    news = requests.get(url='https://seekingalpha.com/api/sa/combined/' + ticker[0] + '.xml')
    
    news1 = requests.get(url='https://newsapi.org/v2/everything?q=' + ticker[1] + '&sortBy=popularity' +'&from=' + date + '&apiKey=' + apiKey[(random.randint(0,9))])
    ...
    df = df.drop(columns=['guid_#text','guid_@isPermaLink','media:thumbnail_@url','sa:picture','sa:stock_sa:company_name','sa:stock_sa:symbol'])

    for index,row in df1.iterrows():
        df1.loc[index, 'sourceName'] = ((((str(row['source']).split("'name': '",1))[1]).split("'}",1))[0])
...
tickers =[["mfst","microsoft"], ["aapl","apple"], ["GME","gamestop"],["amzn","amazon"]]
