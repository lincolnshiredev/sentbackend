...
newsapi = NewsApiClient(api_key=f'api key here')
...
def getNewsApiArticles(ticker: str, date):
    ...
    if(newsApiDf.status.unique() == 'ok'):
        ...
        noResponseData = {'title':['No Data Available'],'url':['N/a'],'sourceName': ['No Source'],'publishedAt': [date],'sentiment':[0],'urlToImage':['"https://images.pexels.com/photos/102720/pexels-photo-102720.jpeg"']}
        # Example stock image, by: Markus Spiske, hosted on: https://www.pexels.com/
        newsApiDf = pd.DataFrame(noResponseData,columns=['title','url','sourceName','publishedAt','sentiment','urlToImage'])
...
def getYahooNewsArticles(ticker: str):
    ...
    if(yahooNews.status_code == 200):
        ...
        noResponseData = {'title':['No Data Available'],'url':['N/a'],'sourceName': ['No Source'],'publishedAt': [date],'sentiment':[0],'urlToImage':['"https://images.pexels.com/photos/102720/pexels-photo-102720.jpeg"']}
        # Example stock image, by: Markus Spiske, hosted on: https://www.pexels.com/
        yahooNewsDf = pd.DataFrame(noResponseData,columns=['title','url','sourceName','publishedAt','sentiment','urlToImage'])
